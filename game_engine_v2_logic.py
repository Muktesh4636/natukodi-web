...
class GameEngine:
    ...
    async def start_new_round(self):
        ...
        # This is now much faster because it usually hits the cache
    ...
    async def publish_state(self, legacy_type=None):
        ...
        # Ensure timer always moves forward and is at least 1
        timer = max(1, int(elapsed_seconds) + 1)
        
        # Calculate end_time (UNIX timestamp)
        # For BETTING phase, it's round_start_unix + betting_close_time
        round_start_unix = int(time.time() - elapsed_seconds)
        
        if self.status == "BETTING":
            end_time = round_start_unix + self.betting_close_time
        else:
            # During other phases, end_time is just current time or result time
