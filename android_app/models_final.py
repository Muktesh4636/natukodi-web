...
class RewardProbability(models.Model):
    """Probability settings for Mega Spin and Daily Reward"""
    TYPE_CHOICES = [
        ('MEGA_SPIN', 'Mega Spin'),
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE, null=True, blank=True, related_name='reward_probabilities', help_text="If null, this is a global setting")
    
    # JSON field to store probabilities: {"10": 0.5, "50": 0.3, "TRY_AGAIN": 0.2}
    # Keys are reward values or types, values are probabilities (0.0 to 1.0)
    probabilities = models.JSONField(default=dict, help_text="Format: {'value': probability}")
    
    updated_at = models.DateTimeField(auto_now=True)

    class Meta:
        unique_together = ['reward_type', 'user']
        verbose_name_plural = "Reward Probabilities"

    def __str__(self):
        scope = f"User: {self.user.username}" if self.user else "Global"
        return f"{self.get_reward_type_display()} - {scope}"
