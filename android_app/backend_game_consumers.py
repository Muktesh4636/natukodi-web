    ...
    async def send_initial_state_background(self):
        ...
        try:
            ...
            if round_obj:
                ...
                # Get settings for is_rolling check
                from .utils import get_game_setting
                roll_time = await database_sync_to_async(get_game_setting)('DICE_ROLL_TIME', 19)
                result_time = await database_sync_to_async(get_game_setting)('DICE_RESULT_TIME', 51)
                ...
                await self.send(text_data=json.dumps({
                    ...
                    'is_rolling': (roll_time <= timer < result_time)
                }))
