        ...
        with db_transaction.atomic():
            ...
            if referrer:
                ...
                if referral_bonus > 0:
                    ...
                    # Update Redis for referrer
                    try:
                        from game.views import redis_client
                        if redis_client:
                            redis_client.set(f"user_balance:{referrer.id}", str(ref_wallet.balance), ex=3600)
                    except: pass
...
def approve_withdraw(request, pk):
    ...
    try:
        with db_transaction.atomic():
            ...
            # Change status immediately to COMPLETED (Payment Completed)
            ...
            Transaction.objects.create(
                ...
                balance_before=balance_before,
                balance_after=wallet.balance,
                description=f"Withdraw approved and completed #{withdraw.id}{f'. {withdraw.admin_note}' if withdraw.admin_note else ''}",
            )
...
def game_settings(request):
    ...
    settings_to_manage = [
        ...
        },
        {
            ...
            'default': '@gundu_support',
            'description': 'Telegram support username or link'
        },
    ]
    current_settings = {}
