...
def get_game_setting(key, default=None):
    ...
    # Check in-memory cache (valid for 60 seconds - increased from 30s)
    if key in _SETTINGS_CACHE and (now - _SETTINGS_CACHE_TIME.get(key, 0)) < 60:
    ...
    try:
        ...
        if result is None:
            # Fallback to settings.py defaults if not in DB
