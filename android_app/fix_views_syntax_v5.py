...
while i < len(lines):
    ...
    # Fix the indentation error in game_stats
        ...
        i += 1
    # Fix the indentation error in last_round_results
    elif 'logger.info("Public last round results API access")' in line and 'try:' in lines[i-1]:
        new_lines.append('        logger.info("Public last round results API access")\n')
        i += 1
    else:
        new_lines.append(line)
        i += 1

with open('views_server_fixed_v5.py', 'w') as f:
